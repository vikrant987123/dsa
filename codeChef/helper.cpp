...
struct compare {// to sort the string vector according to the length
    inline bool operator()(const std :: string& first, const std :: string& second) const{
        return first.size()<second.size();
    }
};

pair<int, int> cyclicSteps(char start, char target) { //cyclic movement of char
    if (!isalpha(start) || !isalpha(target)) return {-1, -1}; // Invalid input check

    start = tolower(start);
    target = tolower(target);

    int forwardSteps = (target - start + 26) % 26;  // Forward movement (clockwise)
    int backwardSteps = (start - target + 26) % 26; // Backward movement (counter-clockwise)

    return {min(forwardSteps, backwardSteps), max(forwardSteps, backwardSteps)};
}
